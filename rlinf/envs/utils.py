...
def _center_crop_image_with_pil(image, crop_scale=0.9, size=(224, 224)):
    if not isinstance(image, Image.Image):
        image = Image.fromarray(np.array(image))
    image = image.convert("RGB")

    crop_scale = min(max(float(crop_scale), 0.0), 1.0)
    ...
    return image.crop((left, top, right, bottom)).resize(size, resampling)
...
def center_crop_image(image):
    ...
    if tf is None:
        return _center_crop_image_with_pil(
            image, crop_scale=crop_scale, size=(224, 224)
        )
