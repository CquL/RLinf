...
class RoboTwinEnv(gym.Env):
    ...
    def _cal_chunk_rewards(self, step_reward, chunk_step, terminations, infos):
        ...
        if self.step_mode == "gen_dense_reward_once":
    ...
    def _to_tensor_1d(self, values, dtype=torch.float32):
        if isinstance(values, torch.Tensor):
            return values.to(device=self.device, dtype=dtype).reshape(-1)
        return torch.as_tensor(
            np.array(values).reshape(-1), dtype=dtype, device=self.device
        )

    def _format_step_reward(self, step_reward, terminations):
        if self.use_custom_reward and self.step_mode != "gen_dense_reward_once":
            return self._calc_step_reward(terminations.float())
        return self._to_tensor_1d(step_reward, dtype=torch.float32)
    ...
    def step(
        ...
        if self.step_mode == "take_action":
            (
                ...
                truncations,
                info_list,
            ) = self._step_with_take_action(actions)
        elif self.step_mode == "gen_dense_reward_once":
            (
                raw_obs,
                step_reward,
                terminations,
                truncations,
                info_list,
            ) = self._step_with_dense_reward(actions)
        elif self.step_mode == "gen_sparse_reward_data":
            raw_obs, step_reward, terminations, truncations, info_list = self.venv.step(
                actions
            )
        else:
            raise ValueError(f"Unsupported RoboTwin step_mode: {self.step_mode}")
        extracted_obs = self._extract_obs_image(raw_obs)
        infos = list_of_dict_to_dict_of_list(info_list)

        terminations = self._to_tensor_1d(terminations, dtype=torch.bool)
        truncations = self._to_tensor_1d(truncations, dtype=torch.bool)

        step_reward = self._format_step_reward(step_reward, terminations)
    ...
    def _step_with_take_action(self, actions):
        if len(self.venv.envs) == 0:
            self.venv._init_envs()

        step_futures = {}
        for env_id in range(self.num_envs):
            future = self.venv.env_thread_pool.submit(
                self._sub_env_take_action_step,
                self.venv.envs[env_id],
                actions[env_id],
            )
            step_futures[future] = env_id

        results = [None] * self.num_envs
        for future in as_completed(step_futures, timeout=120):
            env_id = step_futures[future]
            try:
                results[env_id] = future.result()
            except Exception as exc:
                raise RuntimeError(f"SubEnv {env_id} take_action step error: {exc}")

        return self.venv.transform(results)

    def _sub_env_take_action_step(self, sub_env, actions):
        from robotwin.envs.vector_env import update_obs

        if sub_env.get_instruction() is None:
            sub_env.reset(env_seed=None)

        with sub_env.lock:
            for action in actions:
                sub_env.task.take_action(action)
                if getattr(sub_env.task, "eval_success", False):
                    break

            obs = update_obs(sub_env.task.get_obs())
            obs["instruction"] = sub_env.task.get_instruction()

            success = bool(getattr(sub_env.task, "eval_success", False))
            reached_limit = sub_env.task.take_action_cnt >= sub_env.task.step_lim
            reward = np.array([float(success)], dtype=np.float32)
            termination = np.array([int(success)], dtype=np.int32)
            truncation = np.array([int(reached_limit)], dtype=np.int32)
    ...
    def _step_with_dense_reward(self, actions):
        if len(self.venv.envs) == 0:
            self.venv._init_envs()
        ...
        for env_id in range(self.num_envs):
            future = self.venv.env_thread_pool.submit(
                self._sub_env_dense_reward_step,
                self.venv.envs[env_id],
                actions[env_id],
            )
            step_futures[future] = env_id
        ...
        for future in as_completed(step_futures, timeout=120):
            ...
            try:
                ...
                raise RuntimeError(f"SubEnv {env_id} dense reward step error: {exc}")

        return self.venv.transform(results)

    def _sub_env_dense_reward_step(self, sub_env, actions):
        from robotwin.envs.vector_env import update_obs
        ...
        with sub_env.lock:
            dense_actions = np.asarray(actions)
            if dense_actions.ndim == 2:
                dense_actions = dense_actions[None, ...]

            obs_return, reward, termination, truncation, info = (
                sub_env.task.gen_dense_reward_once(dense_actions)
            )
            raw_obs = obs_return[-1] if obs_return else sub_env.task.get_obs()
    ...
    def chunk_step(self, chunk_actions):
        ...
        if self.step_mode == "take_action":
            (
                ...
                truncations,
                info_list,
            ) = self._step_with_take_action(chunk_actions)
        elif self.step_mode == "gen_dense_reward_once":
            (
                raw_obs,
                step_reward,
                ...
                truncations,
                info_list,
            ) = self._step_with_dense_reward(chunk_actions)
        elif self.step_mode == "gen_sparse_reward_data":
            raw_obs, step_reward, terminations, truncations, info_list = self.venv.step(
                chunk_actions
            )
        ...
        chunk_terminations = torch.zeros(
            (num_envs, chunk_step), dtype=bool, device=self.device
        )
