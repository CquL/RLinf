...
def validate_embodied_cfg(cfg):
    ...
    if cfg.runner.val_check_interval > 0 or cfg.runner.get("only_eval", False):
        ...
        eval_num_action_chunks = (
            1
            if cfg.actor.model.model_type == "act"
            and cfg.actor.model.get("eval_stepwise", False)
            else cfg.actor.model.num_action_chunks
        )
        assert cfg.env.eval.max_steps_per_rollout_epoch % eval_num_action_chunks == 0, (
            "env.eval.max_steps_per_rollout_epoch must be divisible by the eval action chunk size"
        )
